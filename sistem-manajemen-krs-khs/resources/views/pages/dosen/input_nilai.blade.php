        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
            ...
            <div>
                ...
                <select name="kelas"
                    ...
                    <option value="A" {{ request('kelas') == 'A' ? 'selected' : '' }}>
                        Kelas A
                    </option>

                    <option value="B" {{ request('kelas') == 'B' ? 'selected' : '' }}>
                        Kelas B
                    </option>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    ...
    <div class="px-8 py-16 text-center text-gray-400">
        ...
        <p class="text-xs mt-1">
            Coba ubah filter kelas atau mata kuliah
        </p>
    </div>
...
<script>
    ...
    function hitungNilai(rowId) {
        ...
        const tugas = parseFloat(
            document.querySelector(`input[name="tugas[${rowId}]"]`)?.value
        ) || 0;

        const uts = parseFloat(
            document.querySelector(`input[name="uts[${rowId}]"]`)?.value
        ) || 0;

        const uas = parseFloat(
            document.querySelector(`input[name="uas[${rowId}]"]`)?.value
        ) || 0;
